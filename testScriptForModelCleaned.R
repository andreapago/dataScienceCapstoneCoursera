...
input<-file("subsetEnglish005.txt")
...
probabilityWord<-function(listGrmas, wordPreceeding){ #wordPreceeding can be a more words (i.e., gram)
  ...
  while(dime==0){
    ...
    {toReturn<-"Ouch!! There is no word not available in dictionary for the text you entered"
      return(toReturn)
      }else{
...
####to deal with the word cloud####
#########################################
# words<-probabilityWord(list(sixGrams,fiveGrams,fourGrams,threeGrams,twoGrams),"you are")
# DF<-data.frame(as.factor(words))
# dfFreq<-as.data.frame(table(DF))
# wordcloud::wordcloud(dfFreq$DF,dfFreq$Freq,min.freq = 1,)
##########################################
...
subsetData<-function(){
  ...
  selected<-rbinom(length(totalLines),1,0.0025)
  
  fileConn<-file("subsetEnglish0025TestSet.txt")
